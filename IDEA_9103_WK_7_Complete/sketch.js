...
function setup() {
  ...
  imageProperties.aspect = img.width / image.height;
...
function draw() {
  ...
  if (drawSegments) {
    ...
    image(img, 
      imageProperties.xOffset, 
      imageProperties.yOffset, 
...
function calculateImageProperties(){
  ...
  // else; check if the image is longer than the canvas
  ...
  // else; if the image is the same aspect ratio as the canvas, just resize
  else if (imageProperties.aspect == canvasAspectRatio){
    imageProperties.width = width;
    imageProperties.height = height;
    imageProperties.xOffset = 0;
